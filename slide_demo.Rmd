...
Slides can be named as such `name: intro` and then hyperlinked from elsewhere. For eg: `go to [introduction](#intro)` displays as go to [introduction](#intro).
...
```
.size-20[![](assets/landing.png)]  
.size-10[![](assets/landing.png)]  
```
...
`<img src="assets/landing.png" style="width:20%;" class="fancyimage"/>`  
...
```{r,echo=FALSE,comment=""}
cat(paste0("```{r,out.width=250}\n","knitr::include_graphics('assets/landing.png')\n","``` \n"))
```

```{r,echo=FALSE,fig.retina=NULL,out.width=250}
knitr::include_graphics("assets/landing.png")
```
